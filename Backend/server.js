...
app.post('/students', (req, res) => {
    ...
    if (req.body.facility && req.body.facility.length > 0) {
      ...
      facility = commaSeparatedValues.replace(/ /g, "|");

      const commafacility_typeValues = req.body.facility_type.join(' ');
      facility_type = commafacility_typeValues.replace(/ /g, "|");
    }
    ...
    const sql = "INSERT INTO `new_employee`(`employee_id`, `company_id`, `emp_name`, `initial`, `photo`, `direct_no`, `bank_ac`, `bank_details`,`bank_ifsc`, `pan_no`, `aadhar_no`,`aadhar_name`,`aadhar_linkedno`, `passport_no`, `facility`,`vehicle_type`,`licence_type`,`laptop_type`) VALUES (?)";
    ...
    const values = [
        ...
        req.body.licence_type,
        req.body.laptop_type
...
app.get('/employees', (req, res) => {
  ...
  db.query(sql, (err, results) => {
      ...
      db.query(sql2, (err, result2) => {
          ...
          //console.log(combinedResults.employeeData.employee_id);
...
app.listen(8081,()=>{
    console.log("listening on port8001");
})

app.use('/getphoto',express.static(path.resolve(__dirname,'uploads','document')))
app.use('/getphoto1',express.static(path.resolve(__dirname,'uploads')))
