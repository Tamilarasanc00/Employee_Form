...
[]) 
  ...
  console.log(id);
    ...
    const  handlefacilityTypeChange = (e) =>{
      ...
      const step1=values[e.target.name]?values[e.target.name].split("|"):[]
      if (step1.includes(value)) {
        step1.filter((i)=>i!==value)
      } else {
        step1.push(value)
      }
// const fctl=[...addFacilities.facility_type]
// fctl[index]=fctl[index].split(",").includes(value)
console.log(step1,values[e.target.name]);
      SetValues({
        ...values,
        //facility: [...addFacilities.facility, value],
        [e.target.name]: [...step1].join("|")
      });
    }
...
{addFacilities.facility.includes('licence') && (
  ...
  <div className="flex items-right mb-4">
    <input type="text" placeholder="Enter Laptop Model" name='laptop_type' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm  focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400  dark:focus:ring-blue-500 dark:focus:border-blue-500" 
