...
function App() {
  ...
  const [check, setcheck] = React.useState({server:false});
  ...
  useEffect(()=>{
   ...
   async function fun1(params) {
    await server.get("/checking").then((res)=>{if(res.data){setcheck((p)=>({...p,server:false}))};findd=false}).catch((err)=>{setcheck((p)=>({...p,server:true}))})  
    }
    fun1()
    setTimeout(() => {
      if(findd){
        setcheck((p)=>({...p,server:true}))
      }
    }, 3000);
  },[])
  return (
  ...
  <Dialog open={check.server} maxWidth={'sm'} fullWidth={true}><DialogTitle style={{ backgroundColor: "rgb(0,114,188)", color: 'rgb(255,255,255)' }}>Server Not Responding </DialogTitle>
  <DialogContent>
  <h5>Try :</h5>
            <ul>
              <li>Checking the network cables, modem, and router</li>
              <li>Reconnecting to Wi-Fi</li>
              <li>Check the network Speed</li>
            </ul>
            <br/>
          <Button style={{textTransform:"capitalize"}} onClick={async()=>{setcheck((p)=>({...p,checking:true}));
          await server.get("/checking").then((res)=>{setcheck((p)=>({...p,server:false,checking:false}))}).catch((err)=>{setcheck((p)=>({...p,server:true,checking:false}))})}}>{check.checking ?<>Connecting...</>:<>Click here to Connect</>} </Button>
         &nbsp;  {check.checking && <CircularProgress color="inherit" size={20} />}
            <br></br><br/>Otherwise Contact IT Department
  </DialogContent>
  </Dialog></Box>
  );
}
